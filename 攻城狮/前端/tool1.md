笔记
===
<!-- TOC -->autoauto- [保留几位小数](#保留几位小数)auto- [字符串截取](#字符串截取)auto- [字符串分割成数组](#字符串分割成数组)auto- [数组转换字符串](#数组转换字符串)auto- [更多隐藏](#更多隐藏)auto- [网易CSS初始化](#网易css初始化)auto- [选择框设置默认值（jQuery）](#选择框设置默认值jquery)auto- [复制到剪切板](#复制到剪切板)auto- [数组去重](#数组去重)auto- [获取url中的参数](#获取url中的参数)auto- [年-月-日](#年-月-日)auto- [年、月](#年月)auto- [年/月/日/时/分/秒](#年月日时分秒)auto- [年/月/日](#年月日)auto- [调用本地返回键>](#调用本地返回键)auto- [监听断网](#监听断网)auto- [判断是否连网](#判断是否连网)auto- [点击眼睛查看密码](#点击眼睛查看密码)auto- [页面跳转前执行AJAX](#页面跳转前执行ajax)autoauto<!-- /TOC -->
### 保留几位小数
